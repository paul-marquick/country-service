...
public class CountryDataAccess : ICountryDataAccess
{
    ...
    public async Task<List<Country>> SelectListAsync(DbConnection dbConnection, DbTransaction? dbTransaction = null)
    {
        ...
        using NpgsqlCommand dbCommand = new NpgsqlCommand(sql, (NpgsqlConnection)dbConnection, (NpgsqlTransaction?)dbTransaction);
        using NpgsqlDataReader dbDataReader = await dbCommand.ExecuteReaderAsync();
